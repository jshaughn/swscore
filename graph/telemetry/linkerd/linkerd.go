// Package linkerd provides the Linkerd implementation of graph/TelemetryProvider.
package linkerd

// Linkerd.go is responsible for generating TrafficMaps using Linkerd telemetry.  It implements the
...
func BuildNamespacesTrafficMap(o graph.TelemetryOptions, client *prometheus.Client, globalInfo *graph.AppenderGlobalInfo) graph.TrafficMap {
	...
	trafficMap := graph.NewTrafficMap()dst
...
func buildNamespaceTrafficMap(namespace string, o graph.TelemetryOptions, client *prometheus.Client) graph.TrafficMap {
	...
	groupBy := fmt.Sprintf("namespace,daemonset,deployment,k8s_job,replicaset,replicationcontroller,statefulset,dst_namespace,dst_service,dst_daemonset,dst_deployment,dst_k8s_job,dst_replicaset,dst_statefulset,dst_replicationcontroller,destination_%s,destination_%s,status_code,grpc_status_code")
    /* 
	...
	*/
	...
	sourceWorkloadNamespaceQuery := fmt.Sprintf(`namespace!="%s"`, namespace)
